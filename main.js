...
window.addEventListener("DOMContentLoaded", () => {
  // ── Global-ish state (module scoped) ────────────────────────────────────────
  let storyMode = false;
  ...
  let isDrawing = false;
  let selStartPx = null;   // { x, y } in data space
  let selCurPx = null;     // { x, y } in data space
  let selDataRect = null;  // { x1, y1, x2, y2 } in data space
  ...
  function updateCompareHover(event, side) {
    ...
    targetHover.text(
      region
        ? `${region.name} — vegetation proxy: ${valueText}`
        : `Vegetation proxy: ${valueText}`
    );
  }
  ...
  function populateCompareSelects() {
    ...
    compareRightSelect.property(
      "value",
      months.includes("2024-07") ? "2024-07" : months[months.length - 1]
    );
  }
  ...
  // Scrollama is imported and ready if you want to wire story steps to scroll.
});
